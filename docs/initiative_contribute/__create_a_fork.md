...
NOTE: Usually the _default_ branch of a repo is the `main` branch, but it's possible for some repos it could be something else. In GitHub, it's easy to know just by looking at whatever branch is the active one when you navigate to the main repo page, or do a `git clone`.
...
```
git checkout main # Make sure your local repo is on the main branch
git fetch upstream # Scan the remote for any updates
git rebase upstream/main # Rebase my local main on top of the main branch of upstream
```

Your local main branch is now up-to-date. However, this did not update your remote fork. To do that you have to push
